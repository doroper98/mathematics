...
function buildDerivSliders(){
  ...
  } else if(derivFn==='poly3'){
    el.innerHTML=`<div class="yk-panel-title"><div class="yk-dot"></div>변수 조절 Variables</div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-a)"></span><span class="vn" style="color:var(--c-a)">a</span><span class="vd">x³ 계수</span></span><span class="vv" style="color:var(--c-a)" id="val-da">1.00</span></div><input type="range" class="sl-a" id="sl-da" min="-3" max="3" step="0.1" value="1" oninput="updateDeriv()"></div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-b)"></span><span class="vn" style="color:var(--c-b)">b</span><span class="vd">x² 계수</span></span><span class="vv" style="color:var(--c-b)" id="val-db">0.00</span></div><input type="range" class="sl-b" id="sl-db" min="-5" max="5" step="0.1" value="0" oninput="updateDeriv()"></div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-c)"></span><span class="vn" style="color:var(--c-c)">c</span><span class="vd">x 계수</span></span><span class="vv" style="color:var(--c-c)" id="val-dc">0.00</span></div><input type="range" class="sl-c" id="sl-dc" min="-5" max="5" step="0.1" value="0" oninput="updateDeriv()"></div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-d)"></span><span class="vn" style="color:var(--c-d)">d</span><span class="vd">상수항</span></span><span class="vv" style="color:var(--c-d)" id="val-dd">0.00</span></div><input type="range" class="sl-d" id="sl-dd" min="-5" max="5" step="0.1" value="0" oninput="updateDeriv()"></div>`;
  } else {
    el.innerHTML=`<div class="yk-panel-title"><div class="yk-dot"></div>변수 조절 Variables</div>
...
function updateDeriv(){
  ...
  const ddEl=document.getElementById('sl-dd');
  const p={a:+aEl.value,b:bEl?+bEl.value:1,c:cEl?+cEl.value:0,d:ddEl?+ddEl.value:0};
  document.getElementById('val-da').textContent=p.a.toFixed(2);if(document.getElementById('val-db'))document.getElementById('val-db').textContent=p.b.toFixed(2);if(document.getElementById('val-dc'))document.getElementById('val-dc').textContent=p.c.toFixed(2);if(document.getElementById('val-dd'))document.getElementById('val-dd').textContent=p.d.toFixed(2);
  ...
  else if(derivFn==='poly3')eq=`f(x) = ${cv(VC.a,p.a.toFixed(1))}x³ ${ss(p.b)} ${cv(VC.b,af(p.b))}x² ${ss(p.c)} ${cv(VC.c,af(p.c))}x ${ss(p.d)} ${cv(VC.d,af(p.d))}`;
  ...
  const descs={poly:`멱의 규칙: d/dx(xⁿ)=nxⁿ⁻¹\nf'(x)=${(2*p.a).toFixed(1)}x ${ss(p.b)} ${af(p.b)}`,poly3:`f'(x)=${(3*p.a).toFixed(1)}x² ${ss(2*p.b)} ${af(2*p.b)}x ${ss(p.c)} ${af(p.c)}\nf''(x)=${(6*p.a).toFixed(1)}x ${ss(2*p.b)} ${af(2*p.b)}`,sin:`연쇄법칙: f'(x)=${(p.a*p.b).toFixed(2)}cos(${p.b.toFixed(1)}x)`,cos:`연쇄법칙: f'(x)=${(-p.a*p.b).toFixed(2)}sin(${p.b.toFixed(1)}x)`,exp:`지수함수 미분: f'(x)=${(p.a*p.b).toFixed(2)}e^(${p.b.toFixed(1)}x)`,ln:`로그 미분: f'(x)=${p.a.toFixed(1)}/x`};document.getElementById('info-deriv-desc').textContent=descs[derivFn]||'';
}
...
function buildIntegSliders(){
  ...
  if(integFn==='poly'){
    el.innerHTML=`<div class="yk-panel-title"><div class="yk-dot"></div>변수 Variables</div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-a)"></span><span class="vn" style="color:var(--c-a)">a</span><span class="vd">x²</span></span><span class="vv" style="color:var(--c-a)" id="val-iga2">1.00</span></div><input type="range" class="sl-a" id="sl-iga2" min="-3" max="3" step="0.1" value="1" oninput="updateInteg()"></div>
  ...
  } else if(integFn==='poly3'){
    el.innerHTML=`<div class="yk-panel-title"><div class="yk-dot"></div>변수 Variables</div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-a)"></span><span class="vn" style="color:var(--c-a)">a</span><span class="vd">x³</span></span><span class="vv" style="color:var(--c-a)" id="val-iga2">1.00</span></div><input type="range" class="sl-a" id="sl-iga2" min="-3" max="3" step="0.1" value="1" oninput="updateInteg()"></div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-b)"></span><span class="vn" style="color:var(--c-b)">b</span><span class="vd">x²</span></span><span class="vv" style="color:var(--c-b)" id="val-igb2">0.00</span></div><input type="range" class="sl-b" id="sl-igb2" min="-5" max="5" step="0.1" value="0" oninput="updateInteg()"></div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-c)"></span><span class="vn" style="color:var(--c-c)">c</span><span class="vd">x</span></span><span class="vv" style="color:var(--c-c)" id="val-igc2">0.00</span></div><input type="range" class="sl-c" id="sl-igc2" min="-5" max="5" step="0.1" value="0" oninput="updateInteg()"></div>
      <div class="sl-item"><div class="sl-lbl"><span class="nm"><span class="dot" style="background:var(--c-d)"></span><span class="vn" style="color:var(--c-d)">d</span><span class="vd">상수</span></span><span class="vv" style="color:var(--c-d)" id="val-igd2">0.00</span></div><input type="range" class="sl-d" id="sl-igd2" min="-5" max="5" step="0.1" value="0" oninput="updateInteg()"></div>`;
  } else if(integFn==='abs'){
    el.innerHTML=`<div class="yk-panel-title"><div class="yk-dot"></div>변수 Variables</div>
...
function integF(fn,x,p){if(fn==='poly')return x.map(xi=>p.a*xi*xi+p.b*xi+p.c);if(fn==='poly3')return x.map(xi=>p.a*xi*xi*xi+p.b*xi*xi+p.c*xi+p.d);if(fn==='sin')return x.map(xi=>p.a*Math.sin(p.b*xi));if(fn==='cos')return x.map(xi=>p.a*Math.cos(p.b*xi));if(fn==='exp')return x.map(xi=>p.a*Math.exp(p.b*xi));if(fn==='abs')return x.map(xi=>p.a*Math.abs(xi));return x.map(()=>0);}
function integAD(fn,x,p){if(fn==='poly')return x.map(xi=>p.a*xi*xi*xi/3+p.b*xi*xi/2+p.c*xi);if(fn==='poly3')return x.map(xi=>p.a*xi*xi*xi*xi/4+p.b*xi*xi*xi/3+p.c*xi*xi/2+p.d*xi);if(fn==='sin')return x.map(xi=>-p.a/p.b*Math.cos(p.b*xi));if(fn==='cos')return x.map(xi=>p.a/p.b*Math.sin(p.b*xi));if(fn==='exp')return x.map(xi=>p.a/p.b*Math.exp(p.b*xi));if(fn==='abs')return x.map(xi=>p.a*xi*Math.abs(xi)/2);return x.map(()=>0);}
function integADStr(fn,p){if(fn==='poly')return `F(x)=${(p.a/3).toFixed(2)}x³+${(p.b/2).toFixed(2)}x²+${p.c.toFixed(1)}x+C`;if(fn==='poly3')return `F(x)=${(p.a/4).toFixed(2)}x⁴+${(p.b/3).toFixed(2)}x³+${(p.c/2).toFixed(2)}x²+${p.d.toFixed(1)}x+C`;if(fn==='sin')return `F(x)=${(-p.a/p.b).toFixed(2)}cos(${p.b.toFixed(1)}x)+C`;if(fn==='cos')return `F(x)=${(p.a/p.b).toFixed(2)}sin(${p.b.toFixed(1)}x)+C`;if(fn==='exp')return `F(x)=${(p.a/p.b).toFixed(2)}e^(${p.b.toFixed(1)}x)+C`;if(fn==='abs')return `F(x)=${(p.a/2).toFixed(2)}x|x|+C`;return'-';}
...
function updateInteg(){
  ...
  const ddEl2=document.getElementById('sl-igd2');
  const p={a:+aEl.value,b:bEl?+bEl.value:1,c:cEl?+cEl.value:0,d:ddEl2?+ddEl2.value:0};
  document.getElementById('val-iga2').textContent=p.a.toFixed(2);if(document.getElementById('val-igb2'))document.getElementById('val-igb2').textContent=p.b.toFixed(2);if(document.getElementById('val-igc2'))document.getElementById('val-igc2').textContent=p.c.toFixed(2);if(document.getElementById('val-igd2'))document.getElementById('val-igd2').textContent=p.d.toFixed(2);
  ...
  else if(integFn==='poly3')eq=`∫[${cv(VC.a,p.a.toFixed(1))}x³ ${ss(p.b)} ${cv(VC.b,af(p.b))}x² ${ss(p.c)} ${cv(VC.c,af(p.c))}x ${ss(p.d)} ${cv(VC.d,af(p.d))}]dx`;
  ...
  const descs={poly:'다항함수 적분: ∫xⁿdx=xⁿ⁺¹/(n+1)+C',poly3:'3차함수 적분: ∫x³dx=x⁴/4+C',sin:'삼각함수 적분: ∫sin(x)dx=-cos(x)+C',cos:'삼각함수 적분: ∫cos(x)dx=sin(x)+C',exp:'지수함수 적분: ∫eˣdx=eˣ+C',abs:'절댓값 적분: 구간 나눠 계산'};document.getElementById('info-integ-desc').textContent=descs[integFn]||'';
}
